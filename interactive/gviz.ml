...
module MakeEngine
  ...
  (X : sig
     ...
     val get_vertex_list: t -> V.t list
     val get_sg_name: V.t -> int list
   end) =
struct
  ...
  let handle_error f arg =
   ...
   (** [fprint_graph_attribute printer ppf list] pretty prints a list of
    ...
    let fprint_graph ppf graph graph'=

      let vertex_list = (X.get_vertex_list graph) in
      let ctag2str l = Core.Std.List.fold_left ~f:(fun y x -> y ^ (string_of_int x) ^ "a" ) ~init:"" l in
      (*
      let _ = List.iter (fun v -> 
        open_box 0; print_string ("tag in vlist: "); print_string (ctag2str(X.get_sg_name v)); close_box (); print_newline ()) vertex_list 
        in
        *)


      let is_sub l1 l2 = 
        (List.length l1 < List.length l2) && (l1 = fst (Core.Std.List.split_n l2 (List.length l1)))
      in
      let is_direct_sub l1 l2 = 
        (List.length l1 + 1 = List.length l2) && (l1 = fst (Core.Std.List.split_n l2 (List.length l1)))
      in
      ...
      let print_subgraphs ppf =
        let rec print_sub v =
          let subo = ((List.filter (fun vtx -> is_sub (X.get_sg_name v) (X.get_sg_name vtx)) vertex_list)) in
          (*
          let slave = ((List.filter (fun vtx -> is_direct_sub (X.get_sg_name v) (X.get_sg_name vtx)) vertex_list)) in
          let _ = 
            print_newline ();
            open_box 0;
            print_string ("master: ");
            print_string (ctag2str(X.get_sg_name v));
            close_box ();
            print_newline () 
          in
          let _ = 
            open_box 0;
            print_string ("subordinate: ");
            print_int (List.length subo);
            close_box ();
            print_newline () 
          in
          let _ = List.iter (fun v -> 
          ...
          in
          *)
          (if (*(List.length subo > 15)||*)(List.length subo <= 9)||((List.length (X.get_sg_name v) mod 2) = 1)
          then fprintf ppf "@[<v 2>subgraph nocluster_%s { %s;@\n%s;@\n%t };@]@\n"
          else fprintf ppf "@[<v 2>subgraph cluster_%s { %s;@\n%s;@\n%t };@]@\n")
             (* arg1 - cluster tag *)
             (ctag2str (X.get_sg_name v))
             (* arg2 - vertex name *)
             "color=black"
             (* arg3 - vertex name *)
             (X.vertex_name v)
             (* arg4 - recursive call to direct subordinate *)
             (fun ppf ->
                (List.iter
                    (fun v' -> print_sub v')
                    (List.filter (fun vtx -> is_direct_sub (X.get_sg_name v) (X.get_sg_name vtx)) vertex_list)))
        in print_sub (List.hd vertex_list)
    ...
    let output_graph oc graph graph'=
      let ppf = formatter_of_out_channel oc in
      fprint_graph ppf graph graph';
      pp_print_flush ppf ()
