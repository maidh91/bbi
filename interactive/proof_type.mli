...
(* inter_rule : rule used by proof_tree. Has all required information to
   be translated to LBBI rule.  *)
and inter_rule =
...
(* rule : User input data. User pick a world or a proposition by its
   index. *)
and rule =
...
(* [tactic] is a function type.
   For a given [goal], generates sub-goals and its [validation].

   [validataion] requires proof_tree(s) corresponding to sub-goal(s)
   and generates a whole proof_tree.
*)
